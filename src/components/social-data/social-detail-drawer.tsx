...
import type { SocialDataDetail } from "@/types/social-data";
...
interface SocialDetailDrawerProps {
  detail: SocialDataDetail | null;
  onClose: () => void;
}

/** 테이블 행 클릭 시 우측 슬라이드 상세 패널 */
export function SocialDetailDrawer({
  detail,
  onClose,
}: SocialDetailDrawerProps) {
  ...
  return (
    <>
      <div
        ...
        aria-hidden
      />
      <div className="fixed right-0 top-0 z-50 flex h-full max-w-md flex-col border-l border-border bg-card shadow-xl w-full">
