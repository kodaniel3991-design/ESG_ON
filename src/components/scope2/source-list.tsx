...
import type { Scope2EmissionSource } from "@/types/scope2";
import { Button } from "@/components/ui/button";

interface Scope2SourceListProps {
  sources: Scope2EmissionSource[];
  selectedSourceId: string;
  onSelectSource: (id: string) => void;
}

export function Scope2SourceList({
  sources,
  selectedSourceId,
  onSelectSource,
}: Scope2SourceListProps) {
  return (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/40 text-xs text-muted-foreground">
              ...
              <th className="px-4 py-2 text-left font-medium">에너지</th>
